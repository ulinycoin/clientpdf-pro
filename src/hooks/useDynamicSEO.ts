...
import { getToolsSEOData, homepageSEOData, pagesSEOData } from '@/data/seoData';
import { useI18n } from './useI18n';

// Enhanced SEO Head component for dynamic updates
export const useDynamicSEO = (toolKey?: string) => {
  ...
  useEffect(() => {
    const updateSEO = () => {
      ...
      // Get tools SEO data
      const toolsSEOData = getToolsSEOData();
      ...
      if (metaDescription && seoData && seoData.description) {
      ...
      if (ogDescription && seoData && seoData.description) ogDescription.setAttribute('content', seoData.description);
      if (ogUrl && seoData && seoData.canonical) ogUrl.setAttribute('content', seoData.canonical);
      ...
      if (twitterDescription && seoData && seoData.description) twitterDescription.setAttribute('content', seoData.description);
