...
const PDFToImagePage: React.FC = () => {
  ...
  const seoData = getToolSEOData('pdfToImage', language);
  const [toolActive, setToolActive] = useState(false);
  const [result, setResult] = useState<any>(null);

  // Get FAQ data for SEO schema
  const pdfToImageFAQs = getCombinedFAQs('pdf-to-image');

  // Dynamic SEO updates
  useDynamicSEO('pdf-to-image');
  ...
  const handleFileSelect = (selectedFiles: File[]) => {
    // PDF to Image works with single file
  ...
  return (
    <StandardToolPageTemplate
      seoData={seoData}
      toolId="pdf-to-image"
      faqSchema={pdfToImageFAQs.map(faq => ({
