...
export class EnhancedUnicodeFontService {
  ...
  private static readonly UNICODE_REPLACEMENTS = new Map<string, string>([
    ...
    // Специальные символы и пунктуация (ИСПРАВЛЕНО - используем Unicode коды)
    ['\u201C', '"'], ['\u201D', '"'], // Умные кавычки
    ['\u2018', "'"], ['\u2019', "'"], // Умные апострофы
    ['\u2013', '-'], ['\u2014', '-'], // Тире
    ['\u2026', '...'], // Многоточие
  ...
  public static smartCleanText(text: string): string {
    ...
    // Финальная очистка - заменяем оставшиеся проблемные символы
    cleaned = cleaned
    ...
    // Если остались непечатаемые символы, заменяем на ?
  ...
  /**
   * Настройка PDF с оптимальным шрифтом
   */
  public static setupPDFFont(pdf: jsPDF, sampleTexts: string[]): FontSetupResult {
    this.initialize();
    ...
    try {
      ...
      // Пробуем установить рекомендованный шрифт
