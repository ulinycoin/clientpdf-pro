...
import { useSimpleBlogPosts, usePaginatedSimpleBlogPosts, useSimpleBlogSearch } from '../../hooks/useSimpleBlog';
...
export const BlogLayout: React.FC<BlogLayoutProps> = ({
  ...
  const { categories, featuredPosts, recentPosts } = useSimpleBlogPosts(currentLanguage);

  // Use search hook if there's a search query
  const { results: searchResults, hasResults } = useSimpleBlogSearch(searchQuery, currentLanguage);
  ...
  } = usePaginatedSimpleBlogPosts(currentPage, 9, category, tag, currentLanguage);

  const loading = paginationLoading;

  // Use search results if there's a search query, otherwise use regular posts
