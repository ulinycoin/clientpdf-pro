...
// Optimized for minimal main thread blocking time (TBT)
(function setupPolyfills() {
  'use strict';

  if (typeof window === 'undefined') return;

  // Minimal critical polyfills only - defer heavy ones
  ...
  const deferredPolyfills = () => {
    ...
    if (!(window as any).require) {
      (window as any).require = function(moduleName: string) {
        switch (moduleName) {
          case 'stream':
            return {
              Readable: class { constructor() { this._readableState = { objectMode: false }; } },
              Writable: class { constructor() { this._writableState = { objectMode: false }; } },
              Transform: class { constructor() { this._readableState = this._writableState = { objectMode: false }; } }
            };
          case 'buffer':
    ...
    // Upgrade Buffer polyfill if available
